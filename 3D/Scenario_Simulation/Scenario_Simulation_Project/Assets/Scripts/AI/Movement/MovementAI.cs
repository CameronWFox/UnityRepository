...
public class MovementAI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((!movementAgent.pathPending && movementAgent.remainingDistance <= 0.5f) && (movePatrol == true))
        {
           MoveToNextPatrolPoint();
        }
        
        if((!movementAgent.pathPending && movementAgent.remainingDistance <= 0.5f) && (moveWander == true))
        {
            MoveToNextWanderPoint();
        }

        if ((!movementAgent.pathPending && movementAgent.remainingDistance <= 0.5f) && (moveSearch == true))
        {
    ...
    void MoveToNextPatrolPoint()
    {
        ...
        Debug.Log("Heading to patrol point: " + destinationMovementPoint);
    }
    ...
    void MoveToNextWanderPoint()
    {
        ...
        Debug.Log("Heading to next random point: " + movementAgent.destination);
    }
